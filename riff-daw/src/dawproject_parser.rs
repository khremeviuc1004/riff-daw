...
use crate::domain::{
    AudioEffectTrack, AudioPlugin, AudioTrack, DAWItemLength, DAWItemPosition,
    InstrumentTrack, MidiTrack, Note, Project, Riff, RiffReference, RiffSet, Sample,
    SampleReference, Song, Track, TrackEvent, TrackType,
};
...
pub fn write_dawproject(project: &Project, path: &str) -> Result<(), DawprojectParseError> {
        ...
        for content in &structure.content {
            if let ProjectStructureElementTypeContent::Track(track) = content {
                ...
                if let Some(daw_track_id) = &track.id {
                    track_id_map.insert(daw_track_id.clone(), domain_track.uuid().to_string());
                }
...
fn read_track(track: &DawTrackType, reader: &mut DawprojectReader<BufReader<File>>) -> TrackType {
    ...
    if has_instrument {
        ...
        let mut domain_track = InstrumentTrack::new();
        ...
        domain_track.set_instrument(instrument);
        domain_track.set_effects(effects);
        domain_track.set_volume(volume);
        domain_track.set_pan(pan);
        TrackType::InstrumentTrack(domain_track)
    } else if has_audio {
...
fn plugin_from_au(
    ...
    let mut audio_plugin = AudioPlugin::new();
    audio_plugin.set_uuid(uuid);
    audio_plugin.set_name(plugin.device_name.clone());
    audio_plugin.set_descriptive_name(plugin.name.clone().unwrap_or_default());
    audio_plugin.set_format(format.to_string());
    audio_plugin.set_category(device_role_to_string(&plugin.device_role));
    audio_plugin.set_manufacturer(plugin.device_vendor.clone().unwrap_or_default());
    audio_plugin.set_version(plugin.plugin_version.clone().unwrap_or_default());
    audio_plugin.set_file(String::new());
    audio_plugin.set_uid(plugin.device_id.clone().unwrap_or_default());
    audio_plugin.set_is_instrument(plugin.device_role == DeviceRoleType::Instrument);
    audio_plugin.set_plugin_type(plugin_type_from_format(format));
    audio_plugin.set_sub_plugin_id(sub_plugin_id);
...
fn plugin_from_device(
    ...
    let mut audio_plugin = AudioPlugin::new();
    audio_plugin.set_uuid(uuid);
    audio_plugin.set_name(device_name.to_string());
    audio_plugin.set_descriptive_name(String::new());
    audio_plugin.set_format(format.to_string());
    audio_plugin.set_category(device_role_to_string(device_role));
    audio_plugin.set_manufacturer(device_vendor.unwrap_or_default());
    audio_plugin.set_version(String::new());
    audio_plugin.set_file(String::new());
    audio_plugin.set_uid(device_id.unwrap_or_default());
    audio_plugin.set_is_instrument(*device_role == DeviceRoleType::Instrument);
    audio_plugin.set_plugin_type(format.to_string());
...
fn read_preset_data(
    ...
    audio_plugin.set_preset_data(base64::engine::general_purpose::STANDARD.encode(&buf));
}
...
fn notes_to_riff(notes: &NotesType) -> Riff {
    ...
    for note in &notes.note {
        ...
        let mut domain_note = Note::new_with_params(
            0,
            time,
            note.key,
            (velocity * 127.0).round() as i32,
            duration,
        );
        domain_note.set_channel(note.channel as u16);
        events.push(TrackEvent::Note(domain_note));
    }
    let uuid = notes
        .id
        .as_deref()
...
fn domain_to_dawproject(song: &Song) -> (ProjectType, Vec<(String, Vec<u8>)>) {
    ...
    for (track_counter, track) in song.tracks().iter().enumerate() {
        ...
        daw_track_ids.insert(track.uuid().to_string(), daw_track_id.clone());
...
fn plugin_to_device(
    ...
    match format.as_str() {
        ...
        _ => ChannelDevicesElementTypeContent::BuiltinDevice(BuiltinDeviceType {
            name: Some(plugin.name().to_string()),
            color: None,
            comment: None,
            id: Some(plugin.uuid().to_string()),
            device_id: Some(plugin.uuid().to_string()),
            device_name: plugin.name().to_string(),
            device_role: role,
...
fn au_plugin(
    ...
    let device_id = if format == "CLAP" {
        plugin
            ...
            .or_else(|| Some(plugin.uuid().to_string()))
    } else {
        Some(plugin.uuid().to_string())
    };
    AuPluginType {
        name: Some(plugin.name().to_string()),
        color: None,
