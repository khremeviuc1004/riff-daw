...
pub struct RiffSequenceBlade {
    ...
    pub riff_sequence_save_name_btn: Button,
    pub riff_sequence_controls_scrolled_window: ScrolledWindow,
...
impl MainWindow {
    ...
    pub fn setup_track_grid(
        ...
        let changed_event_sender = std::boxed::Box::new(|change: Vec<(Riff, Riff)>, track_uuid: String, tx_from_ui: Sender<DAWEvents>| {
            let _ = tx_from_ui.send(DAWEvents::TrackChange(TrackChangeType::RiffReferenceChange(change), Some(track_uuid)));
        });
        let copied_event_sender = std::boxed::Box::new(|new_riff_references: Vec<Riff>, track_uuid: String, tx_from_ui: Sender<DAWEvents>| {
            let new_riff_refs_data = new_riff_references.iter().map(|riff_reference| (riff_reference.position(), riff_reference.id())).collect_vec();
            let _ = tx_from_ui.send(DAWEvents::TrackChange(TrackChangeType::RiffReferenceDragCopy(new_riff_refs_data), Some(track_uuid.clone())));
        });
        let track_grid_custom_painter = TrackGridCustomPainter::new_with_edit_item_handler(
            state.clone(),
            EditItemHandler::new(
    ...
    pub fn setup_riff_grid(
        ...
        let changed_event_sender = std::boxed::Box::new(|change: Vec<(Riff, Riff)>, track_uuid: String, tx_from_ui: Sender<DAWEvents>| {
            let _ = tx_from_ui.send(DAWEvents::RiffGridChange(RiffGridChangeType::RiffReferenceChange(change), Some(track_uuid)));
        });
        let copied_event_sender = std::boxed::Box::new(|new_riff_references: Vec<Riff>, track_uuid: String, tx_from_ui: Sender<DAWEvents>| {
            let new_riff_refs_data = new_riff_references.iter().map(|riff_reference| (riff_reference.position(), riff_reference.id())).collect_vec();
            let _ = tx_from_ui.send(DAWEvents::RiffGridChange(RiffGridChangeType::RiffReferenceDragCopy(new_riff_refs_data), Some(track_uuid.clone())));
        });
        ...
        {
            let tx_from_ui = tx_from_ui.clone();
            let entry = self.ui.selected_riff_grid_name_entry.clone();
            self.ui.riff_grid_save_name_btn.connect_clicked(move |_| {
                let name = entry.text().to_string();

                if let Ok(name) = name.to_value().get() {
                    match tx_from_ui.send(DAWEvents::RiffGridNameChange(name)) {
                        Ok(_) => (),
    ...
    pub fn setup_piano_roll(
        ...
        {
            let state = state;
            let changed_event_sender = std::boxed::Box::new(|change: Vec<(Note, Note)>, track_uuid: String, tx_from_ui: Sender<DAWEvents>| {
                let horatio = change.iter().map(|(original, changed)| (TrackEvent::Note(original.clone()), TrackEvent::Note(changed.clone()))).collect_vec();
                let _ = tx_from_ui.send(DAWEvents::TrackChange(TrackChangeType::RiffEventChange(horatio), Some(track_uuid)));
            });
            let copied_event_sender = std::boxed::Box::new(|new_notes: Vec<Note>, track_uuid: String, tx_from_ui: Sender<DAWEvents>| {
                let new_notes_data = new_notes.iter().map(|new_note| (new_note.note(), new_note.position(), new_note.length())).collect_vec();
                let _ = tx_from_ui.send(DAWEvents::TrackChange(TrackChangeType::RiffAddNote(new_notes_data), Some(track_uuid.clone())));
            });
            let piano_roll_custom_painter: PianoRollCustomPainter = PianoRollCustomPainter::new_with_edit_item_handler(
                state.clone(),
                EditItemHandler::new(
                    changed_event_sender,
                    copied_event_sender,
    ...
    fn add_riff_sequence_blade(
        ...
        if let Some(riff_item_uuid) = item_uuid.clone() {
    ...
    
        {
            ...
            let reference_item_uuid = item_uuid.clone();
...

        {
            ...
            let entry = riff_sequence_blade.riff_sequence_name_entry.clone();
            riff_sequence_blade.riff_sequence_save_name_btn.connect_clicked(move |_| {
                let name = entry.text().to_string();
...

        {
            let blade = riff_sequence_blade.riff_sequence_blade.clone();
            let tx_from_ui = tx_from_ui.clone();
            riff_sequence_blade.riff_sequence_blade_copy.connect_clicked(move |_| {
                let riff_sequence_uuid = blade.widget_name().to_string();

                match tx_from_ui.send(DAWEvents::RiffSequenceCopy(riff_sequence_uuid)) {
                    Ok(_) => (),
                    Err(error) => debug!("Failed to send riff sequence copy: {}", error),
                }
            });
        }
    ...
    fn add_riff_grid_blade(
...

        {
            ...
            let riff_sequences_box = riff_grid_box.clone();
            let reference_item_uuid = reference_item_uuid.clone();
...

        {
            ...
            let riff_grid_type = riff_grid_type.clone();
            let reference_item_uuid = reference_item_uuid.clone();
            let riff_grid_box = riff_grid_box.clone();
...

        {
            ...
            self.ui.arrangements_combobox.connect_changed(move |arrangements_combobox| {
                if let Some(uuid) = arrangements_combobox.active_id() {
                    ...
                    let _ = tx_from_ui.send(DAWEvents::RiffArrangementSelected(uuid.to_string()));
    ...
    fn add_riff_arrangement_blade(
        ...
        // riff_arrangement_blade.riff_arrangement_scrolled_window.set_vadjustment(Some(&riff_arrangement_vertical_adjustment));
...

        {
            ...
            let entry = riff_arrangement_blade.riff_arrangement_name_entry.clone();
            riff_arrangement_blade.riff_arrangement_save_name_btn.connect_clicked(move |_| {
                let name = entry.text().to_string();
                let uuid = blade.widget_name().to_string();
    ...
    pub fn add_riff_sequence_riff_set_blade(
        &self,
        tx_from_ui: crossbeam_channel::Sender<DAWEvents>,
        riff_sequence_uuid: String,
        riff_set_instance_uuid: String,
        riff_set_uuid: String,
        track_uuids: Vec<String>,
        selected_track_style_provider: CssProvider,
        riff_set_name: String,
        ...
        if let Some((riff_set_heads_box, riff_sets_box)) = riff_set_boxes {
            ...
            let (riff_set_blade_head, riff_set_blade_drawing_areas, blade_box) = MainWindow::add_riff_set_blade(
                ...
                selected_track_style_provider.clone(),
                Some(riff_set_beat_grids.clone()),
                riff_set_instance_uuid.to_string(),
                None,
            );
            ...
            if let Some(selected_child_position) = selected_child_position {
                riff_set_heads_box.set_child_position(&riff_set_blade_head.riff_set_blade, selected_child_position as i32 + 1);
                riff_sets_box.set_child_position(&riff_set_blade_drawing_areas.riff_set_box, selected_child_position as i32 + 1);
            }
    ...
    pub fn find_riff_sequence_riff_set_boxes(
        &self
    ) -> Option<(Box, Box)> {
        let mut found_riff_set_heads_box = None;
        let mut found_riff_sets_box = None;
                    ...
                    for widget in riff_sequence_blade.children().iter() {
                        if let Some(top_level_box) = widget.dynamic_cast_ref::<Box>() {
                            for widget in top_level_box.children().iter() {
                                if let Some(riff_set_scroll_windows_box) = widget.dynamic_cast_ref::<Box>() {
                                    for widget in riff_set_scroll_windows_box.children().iter() {
                                        if widget.widget_name().to_string() == "riff_sequence_riff_set_heads_scrolled_window" {
                                            if let Some(riff_set_heads_scrolled_window) = widget.dynamic_cast_ref::<ScrolledWindow>() {
                                                if let Some(widget) = riff_set_heads_scrolled_window.child() {
                                                    if let Some(view_port) = widget.dynamic_cast_ref::<Viewport>() {
                                                        if let Some(riff_set_heads_box) = view_port.child() {
                                                            if let Some(riff_set_heads_box) = riff_set_heads_box.dynamic_cast_ref::<Box>() {
                                                                found_riff_set_heads_box = Some(riff_set_heads_box.clone());
                                                            }
                                                        }
                                                    }
                                                }
                                                    ...
                                                    if let Some(view_port) = widget.dynamic_cast_ref::<Viewport>() {
                                                        if let Some(riff_sets_box) = view_port.child() {
                                                            if let Some(riff_sets_box) = riff_sets_box.dynamic_cast_ref::<Box>() {
                                                                found_riff_sets_box = Some(riff_sets_box.clone());
                                                            }
                                                        }
                                                    }
                                                }
                                        ...
                                        if let Some(riff_set_heads_box) = &found_riff_set_heads_box {
                                            if let Some(riff_sets_box) = &found_riff_sets_box {
                                                return Some((riff_set_heads_box.clone(), riff_sets_box.clone()));
                                            }
                                        }
                                    }
                                }
    ...
    pub fn update_riff_arrangements_combobox_in_riff_arrangement_view(
        &mut self,
        state: &DAWState,
        restore_selected: bool
    ) {
        ...
        // populate the arrangements combobox
        let mut first = true;
        for riff_arrangement in state.project().song().riff_arrangements().iter() {
            self.ui.arrangements_combobox.append(Some(riff_arrangement.uuid().as_str()), riff_arrangement.name());
    ...
    pub fn update_riff_grids(
        ...
        for riff_grid in state.project().song().riff_grids().iter() {
            ...
            if !restore_selected && first {
                ...
                self.ui.selected_riff_grid_name_entry.set_text(riff_grid.name());
                first = false;
            }
        }
    ...
    pub fn repaint_riff_arrangement_view_active_drawing_areas(
        ...
        if play_position_in_beats <= riff_arrangement_actual_playing_length {
            ...
            for (riff_item_actual_playing_length, riff_item, riff_set_data) in riff_items_actual_playing_lengths.iter() {
                ...
                if running_playing_position <= play_position_in_beats && play_position_in_beats <= riff_item_end_position {
                    if let RiffItemType::RiffSequence = riff_item.item_type() {
                        for (riff_set_actual_playing_length, riff_set_riff_item) in riff_set_data.iter() {
                            let riff_set_end_position = running_playing_position + riff_set_actual_playing_length;
                            if running_playing_position <= play_position_in_beats && play_position_in_beats <= riff_set_end_position {
                                playing_riff_item = Some(riff_set_riff_item.clone());
                                playing_riff_sequence = Some(riff_item.uuid());
                                ...
                                playing_before_riff_item = Some(riff_set_riff_item.clone());
                            }
                            running_playing_position += riff_set_actual_playing_length;
                        }
                    }
                    else if let RiffItemType::RiffGrid = riff_item.item_type() {
                        playing_riff_item = Some(riff_item.clone());
                        playing_riff_grid = Some(riff_item.uuid());
                    ...
                    playing_before_riff_item = Some(riff_item.clone());
                }
                                                            ...
                                                            if let Some(riff_sets_box) = riff_sets_box.dynamic_cast_ref::<Box>() {
                                                                for widget in riff_sets_box.children().iter() {
                                                                    if let Some(riff_set_box) = widget.dynamic_cast_ref::<Box>() {
                                                                        ...
                                                                        if let Some(riff_item_ref) = &playing_before_riff_item {
                                                                            match riff_item_ref.item_type() {
                                                                                RiffItemType::RiffSet => {
                                                                                    // if riff_sequence_or_grid_blade.widget_name().to_string().contains(riff_item_ref.uuid().as_str()) {
                                                                                    //     self.repaint_riff_sequence_riff_set_drawing_areas(riff_sequence_or_grid_blade, playing_riff_item.uuid(), before_riff_item.uuid());
                                                                                    // }
                                                                                }
                                                                                RiffItemType::RiffSequence => {
